...
class Driver(drivers.BaseDriver):
    ...
    def job_client_ack(self, job_id):
        ...
        self.log.debug("Sending job ack for {}".format(job_id))
        self.job_send(msg_id=job_id, control=self.job_ack)

    def job_client_status_send(self, job_id=None, control=None, command=None,
                               data=None, info=None, stderr=None, stdout=None):
        self.log.debug("Sending client status for job {} status: {}".format(
            job_id, control))
        self.job_send(msg_id=job_id, control=control, command=command,
                      data=data, info=info, stderr=stderr, stdout=stdout)
